...
- [~] **会话历史回放**：~~`/history` 卡片回放~~（已实现后**按产品决策移除**：与 `/export` 重复、全量刷卡难看）；会话日志统一由 `/export` 文件消息承担
