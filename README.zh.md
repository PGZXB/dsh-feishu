...
| `/group <名称>` | 和机器人建群（无参会弹名称输入卡） |
| `/cancel` | 停下正在跑的回合 |
| `/cd <路径>` | 设置本聊天的工作目录（无参会弹路径输入卡） |
| `/repo [路径]` | 挑一个项目目录（无参打开选择器） |
| `/status` | 查看本聊天的会话状态 |
| `/feishu-status` | 显示运行状态诊断卡 |
| `/schedule` | 列出活跃的提醒 |
| `/model [provider/model]` | 选模型或设置默认模型（无参打开选择器） |
| `/export` | 把会话日志作为文件发出来 |
| `/sessions` | 列出已保存会话（卡片上可恢复） |
| `/resume [id]` | 在本聊天恢复一个已保存会话（无参打开会话列表） |
| `/clear` `/new` | 开新对话（旧会话仍保留） |
| `/goal <文本>` | 设置任务目标（无参会弹文本输入卡） |
| `/compact` | 压缩较旧的对话历史 |
| `/feedback <文本>` | 发送反馈（无参会弹文本输入卡） |
| `/permission [预设]` | 切换权限预设（无参打开选择器） |
| `/plan [on|off]` | 进入或退出计划模式 |
