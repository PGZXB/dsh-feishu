...
> （当前为 **`0.1.5-rc.2`**）为准构建与验证，从 npm 与从 git 安装都适用。
...
- **会话不丢**——重启守护进程，会话与工作目录都还在；`/sessions` 可列出、恢复、重命名、归档、导出。
- **一次扫码完成接入**——扫一次飞书二维码就建好并配好应用（机器人资料、权限、事件订阅、凭据），并写好 dsh profile。
- **文件双向流动**——你发的图片、文件、视频、富文本都会落到工作区成为普通文件，agent 按路径读取；agent 也能把工作区里的文件/图片发回聊天。
- **消息排队与插话**——回合运行中发的消息会排队可见；可编辑、删除，或插进正在跑的回合。
- **回合产物**——回合产出的文件以 chips 出现在卡片底部，点一下就在聊天里收到。
- **卡片统计与诊断**——卡片上显示回合数、工具调用数、token 与当前上下文占用；`/feishu-status` 显示连接与会话诊断，回合运行中也能用。
- **群聊 + @ 提醒**——群里 @ 机器人即可；出错、审批、提问都会 @ 回发起人。
- **回执、白名单、提醒、导出**——表情回执、`allowedChats` / `allowedUsers` 白名单、定时提醒、会话日志文件。
