...
| Control panel | `/panel` renders every command as a button — no command syntax to remember | ⚙️ panel card with paged, categorized buttons; pickers for model/permission/repo/session — the model picker switches the live session immediately and records the default | ✅ |
