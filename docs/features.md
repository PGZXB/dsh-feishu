...
User-visible feature list and TODO tracker for dsh-feishu. ✅ = shipped,
📋 = planned (roadmap order), 🚧 = in progress. Update this table when a
feature lands.

## Shipped

| Feature | What it does | UX |
|---|---|---|
...
Theme/appearance, message editing, fork sessions, message feedback (👍/👎),
trajectory view, plugin management, global search, session overview page,
skill manager, workflow panel, directory tree, goal bar, todos panel,
plan-review panel, context injection rows — either low value on Feishu
cards, superseded, or out of scope. See `docs/development.md` → "Documentation
map" and the internal gap report for rationale.
