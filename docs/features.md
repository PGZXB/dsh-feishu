...
| One-QR quick setup | `dsh-feishu-setup`: one Feishu QR scan creates/configures the app (bot, scopes, version, credentials) and writes the profile | Terminal-guided QR login with bot branding (name/avatar/description) and optional model API key prompts (the key step skips when one is already configured) | ✅ |
