...
| dsh-feishu track | Ships | Adapted for DSH | How users install |
|---|---|---|---|
| `main` branch | next release's work | **dsh `@next`** (latest pre-release) | git checkout of `main` |
| npm `@latest` (GitHub latest release) | the current stable release | **dsh `@latest`** | `npm i @dsh-feishu/dsh-feishu` |
