...
| dsh-feishu 轨道 | 发布内容 | 适配的 DSH | 用户怎么装 |
|---|---|---|---|
| `main` 分支 | 下一个 release 的工作 | **dsh `@next`**（最新预发布） | git checkout `main` |
| npm `@latest`（GitHub latest release） | 当前稳定 release | **dsh `@latest`** | `npm i @dsh-feishu/dsh-feishu` |
