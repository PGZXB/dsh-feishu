...
dsh-feishu 的功能列表与 TODO 追踪表。✅ = 已实现，📋 = 计划中
（按路线图顺序）。每完成一个功能就更新本表。

| 功能 | 说明 | UX | 状态 |
|---|---|---|---|
| 流式对话卡片 | 一聊一会话；每轮一张卡片原地更新（工具调用、推理、markdown、表格实时流入），终态化为完成/出错/停止 | 流式卡带 ⏹ 停止 / 🔁 重试 / 📋 复制 / ⋯ 行详情 / ▾ 折叠 | ✅ |
| 控制面板 | `/panel` 把每个命令渲染成按钮——无需记忆命令语法 | ⚙️ 面板卡：分类分页按钮；模型/权限/仓库/会话选择器 | ✅ |
| 卡内审批与提问 | 权限升级以卡片形式处理；agent 的提问（单选/多选/自由文本）在聊天里回答 | 🔐 审批卡（允许一次/拒绝）；提问卡（选项/自由文本/取消） | ✅ |
| 会话管理 | 会话跨重启持久化；列表、恢复、重命名、归档/还原、导出、无损开新 | `/sessions` 选择器卡 + 会话详情卡（恢复/重命名/归档/导出）；`/resume`、`/clear` | ✅ |
...
| 反应回执 | 收到消息 → GoGoGo 表情；turn 结束 → DONE/WARN | 两段式反应，可配置，可关闭 | ✅ |
